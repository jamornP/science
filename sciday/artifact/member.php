...
<body class="font-prompt fs-18">
    ...
    <div class="container mt-3">
            ...
            <div class="col-lg-12">
                <div class="shadow-lg p-3 mb-5 bg-white rounded mt-3 fs-20 table-responsive">
                    ...
                    <?php 
                        ...
                        $show = $showroundObj->ShowByActivity(1,1);
                        if($show['edit_data']=='yes'){
                            ?>
                        <div class="d-flex flex-row-reverse bd-highlight mt-3">
                            <a href="/science/sciday/artifact/del.php?id=<?php echo $projects['id']; ?>&stu_id=<?php echo $projects['student_id']; ?>&tea_id=<?php echo $projects['teacher_id']; ?>" class='btn btn-danger text-white'>ลบข้อมูล</a>&nbsp;
                            <button type='button' class='btn btn-warning' data-bs-toggle='modal' data-bs-target='#edit'>
                                <i class='bx bx-edit' ></i> แก้ไขข้อมูล
                            </button>&nbsp;
                                <a href="/science/sciday/success.php?project_id=<?php echo $projects['id']; ?>&activity_id=1&user_id=<?php echo $_SESSION['user_id']; ?>" class='btn btn-success text-white'>ยืนยันข้อมูลถูกต้อง</a>&nbsp;
                                <?php
        ...
        <!-- Round 2 -->
        <?php 
            $show = $showroundObj->ShowByActivity(1,2);
            if($show['showround']=='yes'){
                // echo base64_decode($_REQUEST['artifact_id']);
                $project2s = $artifactObj->getArtifactById(base64_decode($_REQUEST['artifact_id']));
                $round2 = $roundObj->checkRound(base64_decode($_REQUEST['artifact_id']),2,1,$project2s['level_id']);
                if($round2){
                                ...
                                <table class="table table-striped table-hover mt-2 fs-18">
                                    ...
                                    <tbody class="fs-14">
                                        ...
                                        <?php 
                                            ...
                                            $ck2 = $roundObj->checkRound($project2s['id'],2,1,$project2s['level_id']);
        ...
        <!-- Round 3 -->
        <?php 
            $show = $showroundObj->ShowByActivity(1,3);
            if($show['showround']=='yes'){
                $project3s = $artifactObj->getArtifactById(base64_decode($_REQUEST['artifact_id']));
                $round3s = $roundObj->checkRound(base64_decode($_REQUEST['artifact_id']),3,1,$project3s['level_id']);
                if($round3s){
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="video.php" method="post">
                    ...
                    <input type="hidden" class="form-control" id="exampleFormControlInput1" placeholder="" value="1" name="activity_id">
                        <div class="mb-3">
